...
class _StartTripScreenState extends State<StartTripScreen> {
  ...
  Future<void> _confirm() async {
    ...
    tripVm.resetForm();
    tripVm.setTitle(widget.placeName);
    tripVm.setOrigin(LocationModel(
      lat: widget.originLat ?? widget.lat,
      lng: widget.originLng ?? widget.lng,
      label: 'Origem',
    ));
    tripVm.setDestination(LocationModel(
      lat: widget.lat,
      lng: widget.lng,
      address: widget.placeAddress,
      label: widget.placeName,
    ));
    tripVm.setScheduledAt(scheduledAt);

    for (final id in _selectedFriendIds) {
      final friend = friends.where((f) => f.id == id).firstOrNull;
      if (friend != null) tripVm.toggleParticipant(friend);
    }

    final trip = await tripVm.saveTrip();
    if (!mounted) return;

    if (trip != null && _selectedFriendIds.isNotEmpty) {
      final creatorName =
      ...
      try {
        await SupabaseNotificationService.sendInviteNotifications(
          ...
          body: '$creatorName te convidou para uma viagem até "${widget.placeName}"',
          data: {
            'tripId': trip.id,
    ...
    if (trip != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Viagem para "${widget.placeName}" criada!'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Erro ao criar a viagem. Tente novamente.'),
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.location_on, color: AppColors.teal, size: 26),
                        ),
                        ...
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('Destino', style: AppTextStyles.labelSmall.copyWith(color: AppColors.teal)),
                  ...
                  Row(
                    children: [
                      Text('Convidar amigos', style: AppTextStyles.titleMedium),
                      const Spacer(),
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          const SizedBox(width: AppSpacing.sm),
                          Text('Nenhum amigo ainda', style: AppTextStyles.bodyMedium.copyWith(color: AppColors.textMuted)),
                        ],
                      ),
                    )
                  else
...
class _PickerBtn extends StatelessWidget {
  ...
  const _PickerBtn({
    ...
    required this.hasValue, required this.accentColor, required this.onTap,
  });
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: hasValue ? accentColor.withOpacity(0.08) : AppColors.card,
          borderRadius: BorderRadius.circular(AppSpacing.radiusMd),
          border: Border.all(
              color: hasValue ? accentColor.withOpacity(0.4) : AppColors.divider),
        ),
        child: Row(
          children: [
            Icon(icon, size: 18, color: hasValue ? accentColor : AppColors.textMuted),
...
class _FriendTile extends StatelessWidget {
  ...
  const _FriendTile({
    required this.friend, required this.selected,
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: selected ? accentColor.withOpacity(0.08) : AppColors.card,
          borderRadius: BorderRadius.circular(AppSpacing.radiusMd),
          border: Border.all(
              color: selected ? accentColor.withOpacity(0.4) : AppColors.divider),
        ),
        child: Row(
          children: [
            AppAvatar(name: friend.name, imageUrl: friend.avatarUrl, size: 40, showOnline: true, isOnline: friend.isOnline),
            const SizedBox(width: AppSpacing.md),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  if (friend.motoModel != null)
                    Text(friend.motoModel!, style: AppTextStyles.bodySmall.copyWith(color: AppColors.textMuted)),
                ],
              ),
            ),
            AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                    color: selected ? accentColor : AppColors.divider, width: 2),
              ),
              child: selected
